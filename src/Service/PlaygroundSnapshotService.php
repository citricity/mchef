...
final class PlaygroundSnapshotService extends AbstractService {
     ...
     *
     * When $publishOnly is true the snapshot files are published in the mchef-urls repo but
     ...
     */
    public function buildAndPublish(Recipe $recipe, string $slug, bool $publishOnly = false): string {
        if (StaticVars::$instance !== null) {
            return $this->liveSnapshot($recipe, $slug, $publishOnly);
        }
        return $this->freshSnapshot($recipe, $slug, $publishOnly);
    }
     ...
     */
    private function freshSnapshot(Recipe $recipe, string $slug, bool $publishOnly = false): string {
     ...
     */
    private function liveSnapshot(Recipe $recipe, string $slug, bool $publishOnly = false): string {
        $playgroundPath = $this->requirePlaygroundPath();
        $channel        = $this->requireChannel($recipe->moodleTag);
        $gitRef         = preg_match('/^v\d+\.\d+/', $recipe->moodleTag) ? $recipe->moodleTag : '';
